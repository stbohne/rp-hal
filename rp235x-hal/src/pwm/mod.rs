...
//!
//! ```no_run
//! # use rp235x_hal::{prelude::*, gpio::Pins, Sio, pwm::{InputHighRunning, Slices}};
//! # let mut pac = rp235x_pac::Peripherals::take().unwrap();
//! # let pwm_slices = Slices::new(pac.PWM, &mut pac.RESETS);
...
//!
//! ```no_run
//! # use rp235x_hal::{prelude::*, pwm::Slices};
//! # let mut pac = rp235x_pac::Peripherals::take().unwrap();
//! # let pwm_slices = Slices::new(pac.PWM, &mut pac.RESETS);
//! # let mut pwm = pwm_slices.pwm4;
    ...
    M: ValidSliceMode<I>,
{
    ...
    /// Enable the second PWM_IRQ_WRAP interrupt when this slice overflows.
    #[inline]
    ...
    /// Disable the second PWM_IRQ_WRAP interrupt for this slice.
    #[inline]
    ...
    /// Force the second interrupt. This bit is not cleared by hardware and must be manually cleared to
    /// stop the interrupt from continuing to be asserted.
    #[inline]
    ...
    /// Clear force second interrupt. This bit is not cleared by hardware and must be manually cleared to
    /// stop the interrupt from continuing to be asserted.
    #[inline]
...
/// use rp235x_hal::dma::{double_buffer, DMAExt};
/// use rp235x_hal::pwm::{CcFormat, SliceDmaWrite, Slices};
///
/// let mut pac = rp235x_pac::Peripherals::take().unwrap();
///
/// // Init PWMs
...
/// use rp235x_hal::singleton;
/// use rp235x_hal::dma::{double_buffer, DMAExt};
/// use rp235x_hal::pwm::{SliceDmaWrite, Slices, TopFormat};
///
///
/// let mut pac = rp235x_pac::Peripherals::take().unwrap();
///
